...
function getPlanFormatInstructions(
  planHeader: string,
  planFooter: string,
  issueNumber: number,
  readKeyFilesText = "Read key files (README, package.json, tsconfig.json, etc.)",
): string {
  return `## Instructions
...
${getPlanFormatInstructions(planHeader, planFooter, issueNumber)}
`;
}
